...
// miscdevice
struct pushbutton_t {
	struct pushbuttonRegisters_t pushbutton; // device register
	struct miscdevice misc;
	struct platform_device *pdev;	// for dev print
	struct kfifo fifo;				// key press buffer
	struct mutex mutex;				// char dev access
	spinlock_t lock;				// for syncing of kfifo
	wait_queue_head_t waitQueue;	// for waiting on button press
};
...
static irqreturn_t pushbutton_irq_handler(int irq, void *data)
{
	...
	// checking interrupt number and ptr
	if (irq != IRQ_NUMBER || data == NULL) {
		// not adressed to this handler or invalid ptr
		return IRQ_NONE;
	}
	edgeRegisterContent
	...
	if (!kfifo_is_full(&privateData->fifo)) {
		...
		dev_warn(&privateData->pdev->dev, "Button log buffer is full\n");
	// inform wait queue in read fops
	wake_up_interruptible(&privateData->waitQueue);
	// reset interrupt by writing to edge capture register
	...
	size_t count, loff_t *offp)
{
	...
	if (offp == NULL || filep == NULL || buf == NULL) {
		// invalid ptr
		return -EFAULT;
	}
	privateData = container_of(filep->private_data,
	...
	if (*offp > 0 && kfifo_is_empty(&privateData->fifo)) {
		// => read was called at least once
		// and all data was already read
		// => therefore: End of File
...
static int pushbutton_probe(struct platform_device *pdev)
{
	...
	// set device in private data for dev print
	...
	// activate all buttons in interrupt mask register
...
static int pushbutton_remove(struct platform_device *pdev)
{
	...
	// deactivate all buttons in interrupt mask register
